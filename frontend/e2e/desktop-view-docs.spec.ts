...
async function expectTopBarTextToBeReadable(page: Page): Promise<void> {
  // .first(): GlobalFrame's persistent top-bar <header> is always the
  // first <header> in DOM order (AppShell.tsx mounts it ahead of
  // <main>/ShellCanvas); GuidedFirstShow.tsx's own per-stage <header>
  // nests a second one only while the guide overlay is open.
...
async function assertNoRuntimeIssues(page: Page): Promise<void> {
