...
func writeDenoManifest(t *testing.T, root, version, archiveURL, archiveSHA256 string) {
	t.Helper()
	err := os.MkdirAll(filepath.Join(root, "config"), 0o755)
	require.NoError(t, err, "mkdir config: %v", err)
...
`, version, bootstrap.PlatformKey(), archiveURL, archiveSHA256)
	err = os.WriteFile(filepath.Join(root, "config", "toolchain.toml"), []byte(manifest), 0o644)
...
func buildDenoZip(t *testing.T, dir, executableName, body string) (path, digest string) {
	...
	require.NoError(t, err, "create archive: %v", err)
	writer := zip.NewWriter(file)
	header := &zip.FileHeader{Name: executableName, Method: zip.Deflate}
	header.SetMode(0o755)
	entry, err := writer.CreateHeader(header)
	require.NoError(t, err, "create zip entry: %v", err)
	_, err = entry.Write([]byte(body))
	require.NoError(t, err, "write zip entry: %v", err)
	err = writer.Close()
	require.NoError(t, err, "close zip writer: %v", err)
	err = file.Close()
	require.NoError(t, err, "close archive file: %v", err)
	raw, err := os.ReadFile(path)
	require.NoError(t, err, "read archive: %v", err)
...
func denoArchiveFileName(t *testing.T) string {
	...
	default:
		require.Fail(t, fmt.Sprintf("unsupported test platform %q", bootstrap.PlatformKey()))
		return ""
	}
}
...
func TestScopeScriptToolchain(t *testing.T) {
	t.Run("resolves the pinned Deno executable when a verified install exists", func(t *testing.T) {
		...
		err := bootstrap.InstallStaged(archivePath, digest, installDir)
		require.NoError(t, err, "InstallStaged: %v", err)
		...
		require.Equal(t, want, got, "ResolveDenoExecutable = %q, want %q", got, want)
		require.True(t, filepath.IsAbs(got), "ResolveDenoExecutable returned a non-absolute path: %q", got)
	})
	...
	t.Run("fails closed with GOLC_SCRIPT_DENO_MISSING when config/toolchain.toml has no [toolchain.deno] pin", func(t *testing.T) {
		...
		err := os.MkdirAll(filepath.Join(root, "config"), 0o755)
		require.NoError(t, err, "mkdir config: %v", err)
		err = os.WriteFile(filepath.Join(root, "config", "toolchain.toml"), []byte("schema_version = 2\n"), 0o644)
		require.NoError(t, err, "write manifest: %v", err)
		_, err = script.ResolveDenoExecutable(root)
		require.Error(t, err, "expected GOLC_SCRIPT_DENO_MISSING, got %v", err)
		require.Contains(t, err.Error(), "GOLC_SCRIPT_DENO_MISSING", "expected GOLC_SCRIPT_DENO_MISSING, got %v", err)
	})
}
