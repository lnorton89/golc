...
func TestCapabilityDiffMissing(t *testing.T) {
	...
	require.NoError(t, err, "BuildSubstitutionPlan")
	require.True(t, hasWarningSeverity(plan.Warnings, string(substitution.SeverityMissing), string(fixture.CapabilityGobo)), "expected a missing-severity gobo warning, got %+v", plan.Warnings)
	require.Len(t, plan.Errors, 0, "expected no structural errors for a valid target, got %+v", plan.Errors)
}
...
func TestCapabilityDiffIncompatible(t *testing.T) {
	...
	plan, err := substitution.BuildSubstitutionPlan(state, from, to, req)
	require.NoError(t, err, "BuildSubstitutionPlan")
	require.True(t, hasWarningSeverity(plan.Warnings, string(substitution.SeverityIncompatible), string(fixture.CapabilityPan)), "expected an incompatible-severity pan warning, got %+v", plan.Warnings)
}
...
func TestCapabilityDiffUnsupported(t *testing.T) {
	...
	plan, err := substitution.BuildSubstitutionPlan(state, from, to, req)
	require.NoError(t, err, "BuildSubstitutionPlan")
	require.True(t, hasWarningSeverity(plan.Warnings, string(substitution.SeverityUnsupported), string(fixture.CapabilityShutter)), "expected an unsupported-severity shutter warning, got %+v", plan.Warnings)
}
...
func TestSubstitutionNeverApproximates(t *testing.T) {
	...
	assert.True(t, hasWarningSeverity(plan.Warnings, string(substitution.SeverityMissing), string(fixture.CapabilityGobo)), "expected the missing gobo gap to surface, got %+v", plan.Warnings)
	assert.True(t, hasWarningSeverity(plan.Warnings, string(substitution.SeverityIncompatible), string(fixture.CapabilityPan)), "expected the incompatible pan gap to surface, got %+v", plan.Warnings)
	assert.True(t, hasWarningSeverity(plan.Warnings, string(substitution.SeverityUnsupported), string(fixture.CapabilityShutter)), "expected the unsupported shutter gap to surface, got %+v", plan.Warnings)
	require.Len(t, plan.Warnings, 3, "expected exactly 3 warnings (one per gap, none dropped, merged, or silently resolved), got %+v", plan.Warnings)

	// BuildSubstitutionPlan never mutates the input show state: it is a
	// pure computation the author reviews, never an auto-accept.
	require.Len(t, state.Pools[0].Members, beforeMemberCount, "expected BuildSubstitutionPlan to leave the input show state unmutated")
}
...
func TestSubstitutionStructuralError(t *testing.T) {
	...
	plan, err := substitution.BuildSubstitutionPlan(state, from, to, req)
	require.NoError(t, err, "BuildSubstitutionPlan")
	require.Len(t, plan.Errors, 1, "expected exactly one GOLC_SUBSTITUTION_TARGET_INVALID error, got %+v", plan.Errors)
	require.Equal(t, "GOLC_SUBSTITUTION_TARGET_INVALID", plan.Errors[0].Code, "expected exactly one GOLC_SUBSTITUTION_TARGET_INVALID error, got %+v", plan.Errors)
	...
	_, _, _, err = pool.Apply(state.Pools, state.Deployments, state.Groups, plan)
	require.ErrorContains(t, err, "GOLC_POOL_APPLY_PLAN_ERRORS", "expected pool.Apply to refuse a plan carrying a structural error")
}
...
func TestSubstitutionAtomicAcceptCancel(t *testing.T) {
	...
	plan, err := substitution.BuildSubstitutionPlan(state, from, to, req)
	require.NoError(t, err, "BuildSubstitutionPlan")
	require.Empty(t, plan.Errors, "expected a fully compatible substitution to carry no warnings/errors, got warnings=%+v errors=%+v", plan.Warnings, plan.Errors)
	require.Empty(t, plan.Warnings, "expected a fully compatible substitution to carry no warnings/errors, got warnings=%+v errors=%+v", plan.Warnings, plan.Errors)

	// Integrity gate: internal/pool's own ValidatePlanIntegrity, called
	// unmodified against a substitution-produced plan (D-16).
	require.NoError(t, pool.ValidatePlanIntegrity(plan), "expected a freshly built substitution plan to pass pool.ValidatePlanIntegrity")
	tampered := plan
	tampered.Propagate = "immediate"
	err = pool.ValidatePlanIntegrity(tampered)
	require.ErrorContains(t, err, "GOLC_POOL_PLAN_HASH", "expected a tampered substitution plan to fail pool.ValidatePlanIntegrity")

	// Freshness gate: internal/pool's own ValidatePlanFreshness, called
	// unmodified against a substitution-produced plan (D-16).
	require.NoError(t, pool.ValidatePlanFreshness(plan, state.Pools, state.Deployments, state.Groups, state.Revision), "expected a fresh substitution plan to pass pool.ValidatePlanFreshness")
	err = pool.ValidatePlanFreshness(plan, state.Pools, state.Deployments, state.Groups, state.Revision+1)
	require.ErrorContains(t, err, "GOLC_POOL_PLAN_STALE", "expected a stale substitution plan to fail pool.ValidatePlanFreshness")
	...
	require.NoError(t, err, "pool.Apply")
	foundNew := false
	...
	require.True(t, foundNew, "expected the pool to carry a member pinned to the substituted fixture, got %+v", newPools[0].Members)
	for _, m := range newPools[0].Members {
		require.NotEqual(t, member.ID, m.ID, "expected the original member to be removed by the substitution, got %+v", newPools[0].Members)
	}
	...
	// too.
	require.Len(t, state.Pools[0].Members, 1, "expected the original show state to be left unmutated (cancel = discard), got %+v", state.Pools[0].Members)
	require.Equal(t, member.ID, state.Pools[0].Members[0].ID, "expected the original show state to be left unmutated (cancel = discard), got %+v", state.Pools[0].Members)
	...
	require.NoError(t, err, "BuildSubstitutionPlan (revise)")
	require.NotEqual(t, plan.PlanID, revised.PlanID, "expected revising with a different target to produce a distinct plan_id")
}
