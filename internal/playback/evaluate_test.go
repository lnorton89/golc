...
func TestDeterministicEvaluateSameArgs(t *testing.T) {
	...
	require.Equal(t, first, second, "Evaluate called twice with identical args returned different Frames")
}
...
func TestDeterministicEvaluateAcrossGoroutines(t *testing.T) {
	...
	for i, got := range results {
		require.Equal(t, want, got, "goroutine %d: Evaluate not byte-identical", i)
	}
}
...
func TestEvaluateFixedPriorityOverridesPerAttribute(t *testing.T) {
	...
	require.True(t, ok, "expected a Frame entry for instance %s, got %+v", fx.instanceID, frame.Values)
	require.InDelta(t, 0.1, attrs.Values[fixture.CapabilityIntensity], 1e-9, "expected chase step 0's intensity=0.1 to override base-look's 0.5")
	require.InDelta(t, 0.8, attrs.Values[fixture.CapabilityColor], 1e-9, "expected color-theme's color=0.8 to survive untouched")
}
...
func TestEvaluateDisabledLayerContributesNothing(t *testing.T) {
	...
	require.InDelta(t, 0.5, attrs.Values[fixture.CapabilityIntensity], 1e-9, "expected base-look's intensity=0.5 with chase disabled")
}
...
func TestEvaluateChaseStepAdvancesWithPosition(t *testing.T) {
	...
	for _, tc := range cases {
		...
		assert.InDelta(t, tc.wantIntensity, got, 1e-9, "bar %d", tc.bar)
	}
}
...
func TestEvaluateMotionKeyframeSelection(t *testing.T) {
	...
	require.InDelta(t, 0.0, frame0.Values[fx.instanceID].Values[fixture.CapabilityPan], 1e-9, "bar 0: expected pan=0.0")

	pos2 := playback.MusicalPosition{BarIndex: 2, BeatFraction: 0.0}
	frame2 := playback.Evaluate(plan, pos2)
	require.InDelta(t, 1.0, frame2.Values[fx.instanceID].Values[fixture.CapabilityPan], 1e-9, "bar 2: expected pan=1.0")
}
