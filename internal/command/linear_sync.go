...
// linear-sdk-operations` runs the workspace's Node test suite once a later
// plan (01-25, per this plan's own acceptance criteria) adds test files
// under dist/test/. Command resolves the pinned project-local Node
// executable through GOLC_PROJECT_ROOT, the exact environment variable
// golc.ps1 exports before delegating every non-bootstrap route to the
// compiled CLI (cmd/golc-project/main.go); this is the same
// officially-supported-via-the-shim precedent main.go's own root
...
// linearSyncNodeTestCommand resolves the exact `node --test dist/test/`
// invocation this scope's registered Command runs. MustDeclareNodeScope
// requires a non-empty Command at package-init time (before any Request.Root
// is known), so this falls back to GOLC_PROJECT_ROOT -- the same
// environment variable golc.ps1 exports before delegating to the compiled
// CLI -- and, failing that, the process's own working directory. If the
// pinned Node toolchain has not been provisioned yet (bootstrap
// --include linear-sync has not run), this still returns a non-empty
// placeholder Command rather than panicking: MustDeclareNodeScope would
// otherwise crash every route in the CLI at startup over one unbootstrapped
// scope. Invoking this exact scope before bootstrap then fails closed with
// an ordinary "executable file not found" exec error instead.
func linearSyncNodeTestCommand() []string {
	...
	if nodeExecutable, err := resolvePinnedNodeExecutable(root); err == nil {
		return []string{nodeExecutable, "--test", "dist/test/"}
	}
	return []string{"golc-linear-sync-node-not-bootstrapped", "--test", "dist/test/"}
}
