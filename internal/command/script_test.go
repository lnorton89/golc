...
func TestScriptRoutesFullLifecycle(t *testing.T) {
	...
	if err := json.Unmarshal(create.Stdout, &created); err != nil {
		t.Fatalf("unmarshal script create output: %v stdout=%s", err, create.Stdout)
	}
	if created.ID == "" || created.Name != "Chase" {
		t.Fatalf("expected a created script with id/name, got %+v", created)
	}
	if created.CapabilityProfile.Scope != "playback" || created.CapabilityProfile.Preset != "quick-action" {
		t.Fatalf("expected the least-privileged default profile, got %+v", created.CapabilityProfile)
	}
	if created.LastRunStatus != "never_run" {
		t.Fatalf("expected last_run_status never_run, got %q", created.LastRunStatus)
	}
	...
	if dup.ExitCode != 1 || !strings.Contains(string(dup.Stderr), "GOLC_SCRIPT_NAME_DUPLICATE") {
		t.Fatalf("expected GOLC_SCRIPT_NAME_DUPLICATE for a duplicate name, got exit=%d stderr=%s", dup.ExitCode, dup.Stderr)
	}
	...
	if list.ExitCode != 0 {
		t.Fatalf("script list failed: exit=%d stderr=%s", list.ExitCode, list.Stderr)
	}
	var listed []struct {
		ID                string `json:"id"`
		Name              string `json:"name"`
	...
	if err := json.Unmarshal(list.Stdout, &listed); err != nil {
		t.Fatalf("unmarshal script list output: %v stdout=%s", err, list.Stdout)
	}
	if len(listed) != 1 || listed[0].Name != "Chase" || listed[0].LastRunStatus != "never_run" {
		t.Fatalf("expected exactly one listed script named Chase, got %+v", listed)
	}
	if strings.Contains(string(list.Stdout), "\"source\"") {
		t.Fatalf("expected script list to omit source, got: %s", list.Stdout)
	}
	...
	if showResult.ExitCode != 0 {
		t.Fatalf("script show failed: exit=%d stderr=%s", showResult.ExitCode, showResult.Stderr)
	}
	var shown scriptTestView
	if err := json.Unmarshal(showResult.Stdout, &shown); err != nil {
		t.Fatalf("unmarshal script show output: %v stdout=%s", err, showResult.Stdout)
	}
	if shown.ID != created.ID || shown.Source != "" {
		t.Fatalf("expected script show to return the full (empty-source) script, got %+v", shown)
	}
	...
	if afterEdit.ExitCode != 0 {
		t.Fatalf("script show (after edit) failed: exit=%d stderr=%s", afterEdit.ExitCode, afterEdit.Stderr)
	}
	var afterEditView scriptTestView
	if err := json.Unmarshal(afterEdit.Stdout, &afterEditView); err != nil {
		t.Fatalf("unmarshal script show (after edit) output: %v", err)
	}
	if afterEditView.Source != string(sourceBytes) {
		t.Fatalf("expected persisted source to equal the file's bytes verbatim:\nwant %q\ngot  %q", sourceBytes, afterEditView.Source)
	}
	...
	if editMissing.ExitCode != 1 || !strings.Contains(string(editMissing.Stderr), "GOLC_SCRIPT_NOT_FOUND") {
		t.Fatalf("expected GOLC_SCRIPT_NOT_FOUND editing an unknown script, got exit=%d stderr=%s", editMissing.ExitCode, editMissing.Stderr)
	}
	...
	if profileSet.ExitCode != 0 {
		t.Fatalf("script profile set failed: exit=%d stderr=%s", profileSet.ExitCode, profileSet.Stderr)
	}
	var profiled scriptTestView
	if err := json.Unmarshal(profileSet.Stdout, &profiled); err != nil {
		t.Fatalf("unmarshal script profile set output: %v", err)
	}
	if profiled.CapabilityProfile.Scope != "authoring" || profiled.CapabilityProfile.Preset != "long-running-automation" {
		t.Fatalf("expected the updated profile to carry scope=authoring preset=long-running-automation, got %+v", profiled.CapabilityProfile)
	}
	...
	if badScope.ExitCode != 1 || !strings.Contains(string(badScope.Stderr), "GOLC_SCRIPT_SCOPE_INVALID") {
		t.Fatalf("expected GOLC_SCRIPT_SCOPE_INVALID for an invalid scope, got exit=%d stderr=%s", badScope.ExitCode, badScope.Stderr)
	}
	...
	if err := json.Unmarshal(stillValid.Stdout, &stillValidView); err != nil {
		t.Fatalf("unmarshal script show (after bad profile set) output: %v", err)
	}
	if stillValidView.CapabilityProfile.Scope != "authoring" {
		t.Fatalf("expected the invalid profile set to leave the show unchanged, got scope=%q", stillValidView.CapabilityProfile.Scope)
	}
	...
	if deleteResult.ExitCode != 0 {
		t.Fatalf("script delete failed: exit=%d stderr=%s", deleteResult.ExitCode, deleteResult.Stderr)
	}
	afterDelete := registry.Execute(command.Request{Root: root, Args: []string{"script", "list", "--show", showPath}})
	if afterDelete.ExitCode != 0 {
		t.Fatalf("script list (after delete) failed: exit=%d stderr=%s", afterDelete.ExitCode, afterDelete.Stderr)
	}
	if strings.TrimSpace(string(afterDelete.Stdout)) != "[]" {
		t.Fatalf("expected an empty JSON array after delete, got: %s", afterDelete.Stdout)
	}
}
...
func TestScriptCommandBinary(t *testing.T) {
	...
	if result.ExitCode != 0 {
		t.Fatalf("script list on a fresh show failed: exit=%d stderr=%s", result.ExitCode, result.Stderr)
	}
	if strings.TrimSpace(string(result.Stdout)) != "[]" {
		t.Fatalf("expected script list on a fresh show to print exactly [], got: %s", result.Stdout)
	}
}
