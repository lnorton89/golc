...
// "--accept-recovery <id>" (an integer recovery point id), and an optional
// "--discard-recovery" flag (both --flag value and --flag=value forms for
// the id), rejecting anything else and rejecting
// --accept-recovery/--discard-recovery together.
...
// D-10 -- read-only inspect/export/diagnose for that case is a later
// plan's surface), then query show.DetectRecoveryPoints. A newer
...
// beyond the read Load already performs.
func runShowOpen(request Request) Result {
	usage := "show open --show <path> [--accept-recovery <id>] [--discard-recovery]"
	...
	if err != nil {
		...
		if errors.As(err, &tooNew) {
			return Result{ExitCode: 1, Stderr: []byte(fmt.Sprintf("GOLC_SHOW_SCHEMA_TOO_NEW: %v\n", err))}
		}
		return Result{ExitCode: 1, Stderr: []byte(err.Error() + "\n")}
	}
