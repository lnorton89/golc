...
| Standardize the entire Go test suite on `testify` (`require`/`assert`) instead of stdlib `testing.T` assertions | Stdlib's loose comparisons (bare `!=`, hand-rolled `t.Fatalf`) had silently masked several real bugs (inverted `Nil`/`NotNil` checks, `int`-vs-`float64` comparisons) that testify's stricter, typed `require.Equal` catches immediately, plus far more readable failure diagnostics by default | ✓ Good — decided and fully migrated repo-wide 2026-07-31; `require`/`assert` are now the mandatory convention for every new Go test (see AGENTS.md stack table) |
